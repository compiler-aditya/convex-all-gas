...
describe('public landing page', () => {
  ...
  it('uses native links that resolve to real routes', () => {
    const html = page()
    const links = [...html.matchAll(/<a\b[^>]*href="([^"]+)"/g)].map((match) => match[1])
    // The worked example stays reachable without an account.
    expect(links.filter((href) => href === '/demo').length).toBeGreaterThanOrEqual(4)
    // And the real action is present exactly once, in the hero.
    expect(links.filter((href) => href === '/rooms/new').length).toBe(1)
    for (const href of links) {
      if (href.startsWith('#')) {
        // Every in-page anchor must land on something that exists.
        expect(href).not.toBe('#')
        expect(html).toContain(`id="${href.slice(1)}"`)
      } else {
        expect(['/', '/demo', '/rooms/new']).toContain(href)
      }
    }
  ...
  it('labels fictional examples and never presents a signup form', () => {
    ...
    expect(text).toContain('The example needs no sign-up.')
